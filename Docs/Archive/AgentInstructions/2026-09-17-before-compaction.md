...
- Owner clarification, 2026-09-17: Codex may launch the existing applications
  and local project services needed for authorized work without asking again.
  This includes starting Multica when it is offline. For administrative access,
  use Scripts/multica.ps1 -Action StartServices (database/API/web); start its
  task runtime when an execution is intended. This does not select a character,
  authorize new purchases, or dispatch unrelated tasks.

- AI3D task synchronization is verified, 2026-09-17. Multica database/API/web
  were started; runtime remains stopped. MSQ-50 and MSQ-52 through MSQ-60 now
  contain the complete AI3D workflow checklists and current narrow subscription
  exception. MSQ-51 and all other records remain unchanged; statuses, assignments,
  dependencies, stages and all player run histories are preserved. No new issues,
  production run or generation. See Docs/Tasks/PlayerCharacter01AI3DTaskAudit01.md.
  Keep Assets/Source/PlayerCharacter01/ as the canonical editable source root.
  Modeling still requires MSQ-53 named owner selection and the MSQ-52 rig contract.

- Latest owner workflow update, 2026-09-17: the owner has purchased Tripo Studio
  and Meshy subscriptions and identified free Hunyuan3D Studio. The requested
  functional research and protagonist pipeline integration are documented in
  Docs/PlayerCharacter01AI3DPipeline.md and Docs/Research/PlayerCharacter01AI3D/.
  Plan scoped use of those existing web allowances; the older blanket paid-service
  exclusion does not exclude these owner-purchased subscriptions. No new purchases,
  credit top-ups or separately billed API use are authorized. Research spent no
  credits and changed no model assets, DCC/editor state or Multica runs. Use one
  proportional master and production skeleton, separate fitted armor, early
  hand/weapon deformation tests, then final UVs/bakes/Painter. Datum16 is only the
  planning example; named concept selection and the MSQ-52 rig contract remain
  pending before production modeling. Preserve all art and existing owner tests.
  Read the dated provider constraints before execution: Tripo segment before
  quads/rig; Meshy multiview and Smart Topology are separate routes; Hunyuan hosted
  V3.1 exposes 2-8 views but quotas are temporary and hosted commercial-use terms
  still need verification. Multica remains the implementation route.

- Datum16MultiViewTpose01 is delivered after the owner's T-pose correction:
  eight separate 1254 x 1254 white-background PNGs, arms horizontal at shoulder
  height, elbows straight, palms down, with Front/Back/profile/45-degree/axial
  views. Gallery and manifest are under
  Assets/Concepts/PlayerCharacter01/Datum16MultiViewTpose01; see
  Docs/Tasks/PlayerArtDatum16MultiViewTpose01.md. All 75 prior package files,
  including the A-pose set, remain unchanged. Input-art only; owner selection
  and modeling approval remain pending, exact 3D consistency unverified.
  Artist profile/native arguments restored; no active or later task dispatched.

- Datum16MultiView01 is delivered: eight separate white-background PNG views
  (Front, Left45, Right45, Left, Right, Back, Top, Bottom), gallery, exact prompts
  and manifest under Assets/Concepts/PlayerCharacter01/Datum16MultiView01.
  See Docs/Tasks/PlayerArtDatum16MultiView01.md. All 62 prior package files are
  unchanged. This is owner-requested input-art preparation; hidden surfaces
  are inferred and 3D consistency is unverified. Owner selection remains pending;
  no modeling approval, independent art review or external 3D generation.
  Artist profile/native arguments restored; no later task dispatched.

- Datum16TrellisInput01 is delivered: a separate single-figure white-background
  PNG and TRELLIS.2 usage notes under Assets/Concepts/PlayerCharacter01/Datum16TrellisInput01.
  Prior concepts are unchanged. No 3D generation/installation was performed;
  final owner selection remains pending. Artist settings restored.

- Latest owner request, 2026-09-16: prepare operative 16 / Datum as a single
  isolated image for an experimental TRELLIS.2 test and research 3D generation
  tools. Follow Docs/Tasks/PlayerArtDatum16TrellisInput01.md. This is input-art
  preparation, not final concept selection or production modeling approval.
  Preserve all original concepts; owner-only evaluation and existing budget.

- Chevron25Refine03/25C is delivered with four sharper V-shaped overlapping
  cuirass plates. See Assets/Concepts/PlayerCharacter01/Chevron25Refine03/index.html
  and manifest.json. Prior package bytes remain unchanged. MSQ-53 awaits owner
  selection; no independent review or modeling approval. Artist settings restored.

- Latest owner continuation, 2026-09-16: retain four overlapping cuirass plates
  from 25B but restore sharper angular V edges like original 25. Execute 25C via
  Docs/Tasks/PlayerArtChevron25Refine03.md at Astra/max/standard, owner-only
  evaluation. Preserve all previous packages, remaining design and 2043 lore.
  No independent reviewer or final modeling approval.

- Chevron25Refine02/25B is delivered: exactly four broad front and rear cuirass
  plates with shoulder-style overlap. See
  Assets/Concepts/PlayerCharacter01/Chevron25Refine02/index.html and manifest.json.
  The previous 25A interpretation remains owner-rejected history; original 25
  and all earlier package bytes are preserved. MSQ-53 awaits owner selection;
  no independent review or modeling approval. Artist settings restored.

- Latest owner correction, 2026-09-16: rejects 25A's layering interpretation;
  requests exactly four cuirass plates overlapping like existing shoulder
  plates. Execute 25B via Docs/Tasks/PlayerArtChevron25Refine02.md at
  Astra/max/standard, owner-only evaluation. Preserve 25/25A and all history,
  helmet/HUD, 2043 lore and remaining design. No final modeling acceptance.

- Chevron25Refine01/25A is delivered with shallow overlapping chevron cuirass
  plates. See Assets/Concepts/PlayerCharacter01/Chevron25Refine01/index.html
  and manifest.json. Original Concept01/Concept02 bytes remain unchanged.
  MSQ-51 revision delivered, MSQ-53 owner selection still pending; no expert
  review, modeling approval or later dispatch. Artist settings restored.

- Latest owner refinement, 2026-09-16: prefers the freshness of concept 25 and
  requests modest overlapping layers in its cuirass. Execute one focused 25A
  image revision via Docs/Tasks/PlayerArtChevron25Refine01.md, Astra/max/standard,
  owner-only evaluation. Preserve the original 25 and all previous packages,
  helmet/computer/HUD, mobility, 2043 lore and remaining design. This is not
  final modeling approval and dispatches no later task.

- PlayerArt-Concept02/Candidate01 is delivered: ten additional armored concepts,
  IDs 16-25, with closed computer/HUD helmets, based on prior 01/08. See
  Docs/PlayerArtConcept02.md and Assets/Concepts/PlayerCharacter01/Concept02.
  Concept01 remains unchanged. MSQ-51 delivery is complete; MSQ-53 still awaits
  the owner's final named selection without independent expert evaluation.
  Artist profile/native arguments restored; no later gameplay/modeling task.

- Latest owner concept continuation, 2026-09-16: likes Concept01 variants 01
  and 08; requests ten more concepts with increased armor while remaining
  mobile, and a closed helmet containing an onboard computer for the interface.
  Execute MSQ-51 via Docs/Tasks/PlayerArtConcept02.md at Astra/max/standard,
  owner-only art evaluation. Keep 2043, lore, concealed face/eyes and original
  design; preserve Concept01 bytes. New concepts use IDs 16-25. Helmet/HUD is
  ordinary equipment, not the source of powers or restored command contact.
  This is directional preference, not final modeling approval. No later task.

- PlayerArt-Concept01/Candidate01 now contains 15 delivered concept sheets;
  see Docs/PlayerArtConcept01.md and the gallery/manifest under
  Assets/Concepts/PlayerCharacter01/Concept01. MSQ-51 delivery is complete;
  MSQ-53 awaits the owner's named selection with no independent expert run.
  No design is accepted and no later task was dispatched. The Concept Art
  profile/native arguments are restored to the exact pre-task settings.

- Latest owner concept direction, 2026-09-16: execute MSQ-51 for at least 15
  original full-body protagonist concepts, all with fully concealed faces, set
  in the now-fixed year 2043 and grounded in StoryCanon. Use the Concept Art
  profile at Astra/max/standard; existing Multica creative development plus
  controller built-in imagegen rendering is the verified route. The owner alone
  evaluates this batch: no independent concept reviewer or MSQ-53 expert run.
  MSQ-53 is only the named owner-selection gate before modeling. See
  Docs/Tasks/PlayerArtConcept01.md and Docs/Approvals/PlayerArtConcept01-Direction01.json.
  This supersedes the earlier 2-3-direction/high-effort/independent-concept-review
  scope only. No other gameplay or lobby work is dispatched by this request.

- Latest owner gameplay planning direction, 2026-09-16: create tasks for an
  original full-body first-person protagonist and one rifle. MSQ-50 is the new
  parent; MSQ-51 through MSQ-60 are backlog and unassigned, with no run dispatched.
  See Docs/Tasks/PlayerCharacter01Plan.md, Docs/Tasks/PlayerCharacter01Tasks.md and
  Docs/Approvals/PlayerCharacter01-TaskCreation01.json. The protagonist model and
  FP arms must be original; existing/free animations may be reused after review.
  Named concept owner approval precedes modeling. Task creation is not execution
  or visual acceptance; old lobby tasks stay closed and current assets are preserved.

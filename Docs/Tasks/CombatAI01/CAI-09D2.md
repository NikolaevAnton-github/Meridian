# CAI-09D2: Wound gestures and combat action ownership

## Superseded architecture and pending scope: 2026-09-24

This unfinished task is **cancelled as superseded**, not completed. Successors: **MSQ-135**.
...
Hard prerequisites: MSQ-109, MSQ-100.
...
Consume delivered MSQ-100 wound gestures and its declared arm/weapon ownership.
Make conflicting fire/reload/gesture actions mutually exclusive and interruptible
under physical authority. Resume only from valid hand/weapon state, with fresh aim
checks; preserve memory and release stale action/role leases. Verify the affected
gesture-to-combat, recovery/death/reset transitions and no duplicate ammo commit.
Weapon loss/weapon pickup and new healing mechanics are outside this task. MSQ-99 is
not a prerequisite; preserve its independent scope. Owner judges gesture quality.

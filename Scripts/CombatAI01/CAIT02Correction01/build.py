...
assert json.loads((OUT/'checks-02.json').read_text())['passed']
assert not (OUT/'build-result01.json').exists(), 'Build evidence is immutable'
...
with (OUT/'build-guard01.json').open('x',encoding='utf-8') as stream: json.dump(state,stream,indent=2)
...
args=['D:/UE_5.8/Engine/Build/BatchFiles/Build.bat','MeridianSquadEditor','Win64','Development',
      ...
      '-Log='+str(OUT/'build01.log')]
started=datetime.now(timezone.utc).isoformat()
result=subprocess.run(args,cwd=ROOT,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
with (OUT/'build-stdout01.log').open('xb') as stream: stream.write(result.stdout)
...
with (OUT/'build-result01.json').open('x',encoding='utf-8') as stream: json.dump(report,stream,indent=2)

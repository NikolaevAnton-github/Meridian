...
import types,json
from uppervoid01_spatial_unreal import ROOT,OUT,MAP,SOURCE,guard,write,settings
p=ROOT/'Scripts/OpeningLobby/paintermetal01_capture.py'
s=p.read_text().replace('from paintermetal01_unreal import','from uppervoid01_spatial_unreal import')
s=s.replace("['Before','Trial01','Final']","['Before','Disabled','WorldZ','Transmission','Trial01','Final','Restored']")
s=s.replace("lighting='Original owner lights and postprocess; no adjustments'","lighting='All scene lights fixed; dedicated atmosphere diagnostic/correction only'")
...
base.cap.VIEWS['lateral-column-90']=(-1551.150808,330.164361,0,74.912516)
...
def compact_shoot(view):
    ...
    source=source.replace('HighResShot 1920x1080','HighResShot 1920x1082' if view in ['close-column-90','lateral-column-90'] else 'HighResShot 960x540')
    exec(compile(source,str(ROOT/'Scripts/OpeningLobby/reworka01_capture.py'),'exec'),ns)
    ns.update(OUT=base.cap.OUT,VIEWS=base.cap.VIEWS,MAP=MAP)
    record=ns['shoot'](view)
    record['resolution']=[1920,1082] if view in ['close-column-90','lateral-column-90'] else [960,540]
    return record

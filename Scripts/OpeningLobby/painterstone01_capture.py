...
import types
import unreal as u
from painterstone01_unreal import ROOT,OUT,SOURCE,MAP,guard,write,settings
...
cap=types.ModuleType('painterstone01_existing_capture')
exec(compile(source,str(ROOT/'Scripts/OpeningLobby/reworka01_capture.py'),'exec'),cap.__dict__)
cap.VIEWS={'entrance-90':(-1600,350,180,13),
    'full-entrance-90':(1900,0,180,0),
    'entrance-clear-90':(-1500,0,180,18),
    'portal-complete-90':(-1400,0,180,21),
    'sample-context-90':(-700,430,180,17),
    'column-near-90':(-1540,680,0,4),
    'front-return-90':(-1610,410,37,7),
    'grazing-90':(-1430,500,17,0)}
active=None
...
def run(operation,argument):
    ...
    assert folder in ['Trial01','Final','FinalContext']
    candidate=not folder.startswith('Before')

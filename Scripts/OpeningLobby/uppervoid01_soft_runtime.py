...
import unreal as u
from uppervoid01_soft_unreal import ROOT,OUT,MAP,guard,write,settings
...
class AtmosphereWalk(WalkCheck):
    ...
    def __init__(self):
        ...
        self.plan=[('wait','spawn',2),('move','entrance_clear',-2700,-105),('move','lane_align',-2700,465),('move','lane_inbound',-1900,465),('move','axis_align',-1900,0),('move','column_approach',-1560,0),('move','near_column',-1560,240),('look','near_column_up',0,75),('capture','close_column_up',7),('look','nearby_turn',22,68),('wait','turned_up',3),('look','restore_column',0,0)]
    def tick(self,dt):
        if self.done:return
        try:
            age=time.monotonic()-self.started
            if age-self.last_frame>=3:

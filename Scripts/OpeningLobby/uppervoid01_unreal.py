...
def action(operation,argument=''):
    ...
    if operation=='initial':
        s=guard(True,True,pie=True);return write('initial-live-state',s)
    if operation=='stop':
    ...
    if operation.startswith('capture_'):
        import uppervoid01_capture as cap
        return cap.run(operation[8:],argument)
    if operation=='build':
        import uppervoid01_material as m
    ...
    if operation.startswith('runtime_'):
        import uppervoid01_runtime as r

...
OUT=ROOT/'Saved/OpeningLobby/UpperVoid01/HeightCorrection04/Worker'
...
def action(operation,argument=''):
    ...
    if operation.startswith('capture_'):
        import uppervoid01_spatial_capture as cap
        return cap.run(operation[8:],argument)
    if operation=='build':
        import uppervoid01_spatial_material as m
    ...
    if operation.startswith('runtime_'):
        import uppervoid01_spatial_runtime as r

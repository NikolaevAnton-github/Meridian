...
OUT=Path(__file__).resolve().parents[2]/'Saved/OpeningLobby/UpperVoid01/Worker'
def work(operation,argument=''):
    r=call('call_tool',dict(toolset_name='Game.Scripts.OpeningLobby.uppervoid01_tools.OpeningLobbyUpperVoid01Tools',tool_name='action',arguments=dict(operation=operation,argument=argument)))
...
if __name__=='__main__':
    if sys.argv[1]=='captures':captures(sys.argv[2],sys.argv[3:])
        ...
        p=OUT/'Final/entrance-axis-90-camera.json';original=p.read_bytes()
        try:

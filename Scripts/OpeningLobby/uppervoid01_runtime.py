...
import unreal as u
from uppervoid01_unreal import ROOT,OUT,MAP,guard,write,settings
...
class AtmosphereWalk(WalkCheck):
    ...
    def __init__(self):
        folder=OUT/'Movement02';folder.mkdir(exist_ok=True)
        ...
        self.plan=[('wait','spawn',2),('move','entrance_clear',-2700,-105),('move','lane_align',-2700,465),('move','lane_inbound',-1900,465),('move','axis_align',-1900,0),('move','enter_hall',-1600,0),('move','central_route',0,0),
            ...
            ('move','cross_to_west',1680,-980),('move','west_return',-1680,-980),('move','center_return',-1680,0),('move','return_axis',-1900,0),('move','return_lane_align',-1900,-465),('move','return_lane_out',-2700,-465),('move','return',-2800,0)]
    def tick(self,dt):
        if self.done:return
        try:
            age=time.monotonic()-self.started
            if age-self.last_frame>=8:
